...
export const useInventory = (user: User) => {
    ...
    useEffect(() => {
        SystemService.getShifts().then(data => {
            const loadedShifts = data && data.length > 0 ? data : INVENTORY_CONFIG.SHIFTS;
    ...
    const handlePrint = useCallback(() => {
        ...
        const currentShiftInfo = shifts.find(s => s.id === shift) || INVENTORY_CONFIG.SHIFTS[0];
        const now = new Date();
    ...
    const currentShift = shifts.find(s => s.id === shift) || INVENTORY_CONFIG.SHIFTS[0];
