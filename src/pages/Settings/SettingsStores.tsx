...
export const SettingsStores: React.FC<SettingsStoresProps> = ({ toast, initialStores }) => {
    ...
    const handleSaveStore = async (index: number) => {
        ...
        setSaving(true);
        try {
            const res = await SystemService.saveStore({ ...draftStore, sort_order: draftStore.sort_order ?? index });
    ...
    const handleRemoveStore = async (index: number) => {
        ...
        if (store.id) {
            setConfirmDialog({
                title: 'Xóa cửa hàng',
                message: `Bạn có chắc xoá cửa hàng "${store.name || store.code}"? (Dữ liệu liên quan có thể bị ảnh hưởng)`,
                onConfirm: async () => {
    ...
    const handleStoreDrop = async (e: React.DragEvent<HTMLTableRowElement>, targetIndex: number) => {
        e.preventDefault();
        setDragStoreOverIndex(null);
        ...
        // Update sort_order for all stores
        const reorderedStores = newStores.map((s, idx) => ({ ...s, sort_order: idx }));
        ...
        const storesWithIds = reorderedStores.filter(s => s.id);
        if (storesWithIds.length > 0) {
            const res = await SystemService.reorderStores(
                storesWithIds.map(s => ({ id: s.id, sort_order: s.sort_order! }))
            );
            if (!res.success) {
                                    ...
                                    >
                                        ...
                                        <td>
                                            <div className="stg-row-actions" style={isEditing ? { opacity: 1 } : undefined}>
                                                {isEditing ? (
                                                    <>
                                                        <button onClick={() => handleSaveStore(i)} className="stg-btn-icon stg-btn-save" title="Lưu vào hệ thống" disabled={saving}>
                                                            {saving
